...
export default function TablaInformacion({
  ...
  return (
    <table className={estilos.tabla}>
      ...
      <tbody>
        ...
        : infUsers.length === 0 ?
        <tr className={estilos.filastr}>
            <td className={estilos.filas}>No hay publicaciones disponibles en este momento</td>
          </tr>
        : 
        infUsers && (
  ...
  (!mostrarUsuariosEliminados && !e.eliminado_en)  ?
              <>
                ...
                <td className={estilos.filas}>
                  {detallesUsuarios &&
                  (`${detallesUsuarios[index].nombre_usuario} ${detallesUsuarios[index].apellido_usuario}`)
                  }
                  </td>
                <td className={estilos.filas}><div className={estilos.contenido}>{e.titulo_publicacion}</div></td>
  ...
  datosMultimedia && datosMultimedia
    .filter((m: any) => m.id_publicacion === e.id_publicacion)
    .map((m: any) => (
      <div key={m.id_multimedia} className={estilos.links} >
        <Link href={m.ruta_multimedia} target="_BLANK">{m.ruta_multimedia}</Link>
      </div>
    ))
    }
</td>
