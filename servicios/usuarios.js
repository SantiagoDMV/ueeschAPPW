  ...
  try {
    ...
    for (const usuario of usuariosAmbosSistemas) {
      ...
      try {
        ...
        const usuarioRepo = new UsuariosRepository();
        const respuestaCreacionDB = await usuarioRepo.crearUsuarioAmbos(
          idMoodle,
          hashingPassw,
          usuario.id_rol,
          usuario.nombre_usuario,
...
export const registrarUsuarios = async (req) => {
  try {
    ...
    let consulta;
    if (!id_rol) {
      consulta = await usuarioRepo.crearUsuario(hashingPassw, 5, userData);
    } else {
      const idRol = parseInt(id_rol);
      consulta = await usuarioRepo.crearUsuario(hashingPassw, idRol, userData);
    }
...
export const actualizarUsuariosImportados = async (req) => {
  try {
    ...
    const hashingPassw = await hashing(password_usuario);
    const usuarioRepo = new UsuariosRepository();
    const consulta = await usuarioRepo.actualizarUsuarioPassword(
      id_rol,
      cedula_usuario,
      email,
      hashingPassw
...
export const eliminarUsuarios = async (req) => {
  ...
  try {
    let i = 0;
    try {
      const publicacionesRepo = new PublicacionRepository();
      ...
      for (const e of userEmails) {
        if (parseInt(userRoles[i]) === 3 || parseInt(userRoles[i]) === 4) {
          ...
          await usuarioRepo.eliminacionLogicaPorIdMoodle(parseInt(e)); //ACTUALIZACION DE LA CUENTA DE PADRES
    ...
    const consulta = await usuarioRepo.eliminarUsuario(userIds);
...
export const actualizarImagenUsuarios = async (req, res) => {
  try {
    ...
    const usuarioRepo = new UsuariosRepository();
    const consulta = await usuarioRepo.actualizarImagenUsuario(
      user.id,
      datos.imagen
    );
...
export const actualizarFechaEliminacion = async (req, res) => {
  try {
    ...
    const usuarioRepo = new UsuariosRepository();
    const consulta = await usuarioRepo.actualizarFechaEliminacion(
    ...
    const consultaUsuario = await usuarioRepo.buscarUsuarioId(
...
export const actualizarIdMoodlePadres = async (req, res) => {
  try {
    ...
    const consulta = await usuarioRepo.buscarUsuarioEmailNoEliminados(
      emailHijo
    );
    ...
    if (consulta.id_moodle && consulta.id_rol === 4)
      consultaActualizar = await usuarioRepo.actualizarIdMoodle(
        consulta.id_moodle,
        user.email
      );
